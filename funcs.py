...
name_lu = dict(vml=vml_name, cblas=cblas_name, ipp=ipp_name)
...
class MklHeader():
    ...
    def impl(self,t):
        params = [get_call(n,ty) for n,ty in self.params if (ty != 'IppHintAlgorithm') or (t=="Float")]
        pstr = ','.join(params)
...
def test_parse() :
    ...
    cblas1_exp_decl = "asum(_ N:Int, _ X:PtrT, _ incX:Int)->T"
    ipp1_exp_decl = "stdDev(_ pSrc:PtrT, _ len:Int, _ pStdDev:MutPtrT)"
    ipp2_exp_decl = "addC(_ pSrc:PtrT, _ val:T, _ pDst:MutPtrT, _ len:Int)"
